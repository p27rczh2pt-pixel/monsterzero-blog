...
Do prices across a whole area move together, on the same day, by close to the same amount, even though the stations are run by different companies? That's the pattern this lawsuit is built around.
Has genuine price variation between nearby stations gotten harder to find over the past couple years? A market with real competition still shows some spread.
Watch for state-level legislation similar to California's AB 325. As this case gets attention, expect other states to consider similar bills extending antitrust law to cover algorithmic coordination.
